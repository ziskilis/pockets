...
function AddWidgetModal({ active, setActive }) {
  ...
  return (
    ...
    >
      <div
        className="add-widget-modal__content"
        onClick={(e) => e.stopPropagation()}
      >
        <h2>Добавить виджет</h2>
        <input
          type="number"
          className="add-widget-modal__items"
          value={widget.amount}
          name="amount"
          placeholder="Сумма"
          onChange={(e) => changeWidgetHandler(e)}
        />
        <select
          className="add-widget-modal__items"
        ...
        <button
          className="add-widget-modal__button"
          onClick={() => addWidgetHandler(widget)}
        >
          Добавить
        </button>
      </div>
